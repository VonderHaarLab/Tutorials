...
### This beginner-friendly tutorial will allow you to perform data manipulation and create publication-level graphs for 16s microbiome data in R. Below you will find code for extracting alpha diversity, beta diversity, and taxa abundance. 

### The original code and data files can be found at: https://github.com/mfrankz/microbiome. 
...
library(dplyr)
```
2. Import a phyloseq object. This is a multidimensional R object that contains bacterial DNA sequences and associated taxonomy assignments, along with sample information (e.g., grouping variables). We will use the phyloseq object created after the data pre-processing from the [dada2 tutorial for R users](https://benjjneb.github.io/dada2/tutorial.html). You can create the object yourself using the dada2 tutorial or import mine [here](https://github.com/mfrankz/microbiome/blob/main/ps.rds). 
...
1. Create a basic alpha diversity plot. This is the type of plot you will find in most phyloseq tutorials.
```
...
3. Extract alpha diversity data into a dataframe for easier manipulation and analyses. The dataframe alpha_df contains a row for each sample with the metadata (SampleID, Subject, Gender, Day, When) and Shannon alpha diversity score.
```
...
```
4. Analyze beta diversity scores using PERMANOVA (the non-parametric version of MANOVA, which allows for analysis of multiple outcomes). To conduct PERMANOVA, we will need to create 2 separate dataframes: one that contains all possible independent variables (i.e., the sample data) and one that contains the dependent variables (beta diversity scores).
```
...
# 3. Taxa Abundance
### The next step is to determine how changes in specific levels of taxonomy may be driving these broader changes in alpha and beta diversity. Below, we will quantify taxa abundance at the level of the phylum, but please note that you can easily change this to other levels of taxonomy (e.g., Genus, Family).
